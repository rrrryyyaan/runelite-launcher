...
APPBASE="build/macos-aarch64/Exiles.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/Exiles.jar
    ...
    cp native/build-aarch64/src/Exiles $APPBASE/Contents/MacOS/
    cp build/libs/Exiles.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on Exiles
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/Exiles
    popd

    otool -l $APPBASE/Contents/MacOS/Exiles
}
...
dmg() {
    ...
    mv Exiles\ *.dmg Exiles-aarch64.dmg

    # dump for CI
    hdiutil imageinfo Exiles-aarch64.dmg

    if ! hdiutil imageinfo Exiles-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo Exiles-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Exiles-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Exiles-aarch64.dmg
    fi
}
